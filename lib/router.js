...
module.exports = (server) => {
  ...
  return async (req, res) => {
    try {
      ...
      // static vue.js
      if (req.url === '/vue.js') {
        const file = prod && !debug ? 'vue.min.js' : 'vue.js'
        const buf = await fs.readFile(`${__dirname}/../../vue/dist/${file}`)
      ...
      if (req.url === '/vue-router.js') {
        const file = prod && !debug ? 'vue-router.min.js' : 'vue-router.js'
        const buf = await fs.readFile(`${__dirname}/../../vue-router/dist/${file}`)
      ...
      // static axios.js
      if (req.url === '/axios.js') {
        const file = prod && !debug ? 'axios.min.js' : 'axios.js'
        const buf = await fs.readFile(`${__dirname}/../../axios/dist/${file}`)
