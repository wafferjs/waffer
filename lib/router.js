...
module.exports = (server) => {
  ...
  const serve_module = async (req, name, fdebug, dir) => {
      if (req.url === `/${name}.js`) {
        let file = `${name}.js`
  ...
  return async (req, res) => {
    try {
      ...
      // serve modules
      serve_module(req, 'vue', true)
      serve_module(req, 'vue-router', true)
      serve_module(req, 'axios', true)
      serve_module(req, 'vue-progressbar', 'vue-progressbar.js')
